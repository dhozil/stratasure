...
- Implemented steward-requested issuance and settlement safeguards: coverage starts cannot be retroactive, product rules cap premium/payout and payout-to-premium multiple, evaluation has a 30-day grace period before expiry, and NASA POWER responses must contain every required daily key. Added focused direct and integration tests; direct suite passes 30 tests, while Studio integration remains unverified because Studio was offline and the local GLSim process did not progress past deployment before it was stopped.
- Redeployed the safeguarded contract to Studionet at `0x3236D69EEdE1571091D9B4607C85A7790c1593D6`; transaction `0x8b5cdd1c09a688fe7ecf0337bcf44c796b00bad2afbe9ccf4b4554a4ec755088` finalized with `MAJORITY_AGREE` and leader `SUCCESS`; deployed schema, code, and `get_contract_info` rules were verified through GenLayerJS. Updated the local frontend contract address.
- Funded smoke checks against the new deployment confirmed retroactive issuance and unbounded payout rollback, bounded future policy creation, pre-coverage evaluation/expiry rejection, and the new policy/accounting state. A post-coverage evaluation requires waiting for a newly issued policy to reach its observation window.
